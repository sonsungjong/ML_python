...
def plot_ts_anoms(inDF, savepath):
    ...
    if 'expected_value' in inDF.columns:
        ...
        markeredgewidth=1, markerfacecolor='None', markeredge='c', label='expected_value')
    plt.legend();
    plt.xlabel('time');
    plt.ylabel('value');

results = detect_ts(co2, max_anoms=0.01, direction='both')
...
# 알고리즘을 2개 이상 섞어 사용하여 잘 감지하는 부분에 대해서 담당을 시킬 수 있음